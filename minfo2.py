...
def CheckEqual(l1,l2):
    ...
    print len(smaller)
    print len(bigger)
    print len(bigger-smaller)
    return np.array(list(smaller)) 
...
def main():
    ...
    for Lx, dic in Lxs.items():
        for geom, rscalar in reversed(dic.items()):
            ...
            Beta = Beta[shift:]
            if j>0: 
                  #If betas are not equal, consider only the subset of smaller ones
                  print "Unequal beta lists"
                  #subBeta = CheckEqual(Beta2,Beta[1:])
                  print len(Beta)
                  submask = np.where(np.in1d(Beta2.ravel(),Beta[1:]))
                  
                  print "Mask length: ", submask
                  #E     = E[submask]
                  #dE    = dE[submask] 
                  #Beta  = Beta[submask]
            Beta2 = Beta
